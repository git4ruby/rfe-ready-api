...
RSpec.describe EvidenceChecklistPolicy, type: :policy do
  ...
  let(:item) { create(:evidence_checklist, tenant: tenant, case: rfe_case, rfe_section: section) }
  ...
  permissions :update?, :toggle_collected? do
    ...
    it "denies viewer" do
      expect(subject).not_to permit(viewer, item)
    end
  end
end
